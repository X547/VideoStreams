...
image_id load_accelerant_clone(GetAccelerantHook &fGetAccelerantHook)
{
	image_id fAddonImage = -1;
...
public:
...
#else
	fImage.SetTo(load_accelerant_clone(fGah));
#endif
		if (!fImage.IsSet()) {
	...
	{
		// printf("fDm.timing.v_display: %d\n", fDm.timing.v_display);
		// printf("DisplayBufferId(): %d\n", DisplayBufferId());
		// printf("mda(%d, %d)\n", 0, fDm.timing.v_display*DisplayBufferId());
